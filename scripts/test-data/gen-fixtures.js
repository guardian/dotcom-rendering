...
try {
  const requests = articles.map(article => {
    return fetch(`${article.url}.json?dcr`)
      ...
      .then(json => {
        ...
        const contents = `${HEADER}export const ${article.name}: CAPIType = ${JSON.stringify(json, null, 4)}`
        fs.writeFileSync(`${root}/fixtures/articles/${article.name}.ts`, contents, 'utf8')
  ...
  Promise.all(requests).finally(() => {
    console.log('\nFormatting files...')
    execa('prettier', ['./fixtures/**/*', '--write', '--loglevel', 'error']).then(() => {
