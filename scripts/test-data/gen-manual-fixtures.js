...
const jsonPath = resolve(__dirname, 'data');
...
try {
	...
	// Copy manual fixtures
	fixtures.forEach((fixtureName) => {
		const data = fs.readFileSync(`${jsonPath}/${fixtureName}.json`);
		const json = JSON.parse(data);
		const contents = `${MANUAL_HEADER}export const ${fixtureName} = ${JSON.stringify(
			json,
			null,
			4,
		)}`;
		...
		fs.writeFileSync(
			`${root}/fixtures/manual/${camelToUnderscore(fixtureName)}.ts`,
			contents,
			'utf8',
		);
	});
	...
	execa('prettier', [
		...
		console.log(`\nDone ✅ Successfully created manual fixtures\n`);
	});
} catch (e) {
	console.error(e);
}
