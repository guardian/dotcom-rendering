...
import { providePromotedNewsletter } from './newsletterData';
...
async function serveEditionsArticlePost(
	...
	try {
		...
		const enhancedRenderingRequest = await providePromotedNewsletter(
			renderingRequest,
			content,
		);
		const themeOverride = themeFromUnknown(req.query.theme);
		await serveEditionsArticle(
			enhancedRenderingRequest,
			res,
			themeOverride,
		);
	} catch (e) {
	...
	try {
		...
		await either(
			...
			async ([content, relatedContent]: [Content, RelatedContent]) => {
				...
				const enhancedMockedRenderingRequest =
					await providePromotedNewsletter(
				...
				if (richLinkDetails) {
					await serveRichLinkDetails(
						enhancedMockedRenderingRequest,
						res,
					);
				} else {
					const page: Option<string> = getPageId(req.query);
