...
describe('Liveblogs', function () {
	...
	it('should show the toast, incrementing the count as new updates are sent', function () {
		stubUpdates();
		cy.visit(`/Article/web/${blogUrl}?live=true`);
		// Wait for hydration
	...
	it('should use the right block id when polling from the second page', function () {
		...
		cy.visit(
			`/Article/web/${blogUrl}?live=true&page=with:block-6214732b8f08f86d89ef68d6&filterKeyEvents=false#liveblog-navigation`,
		);
		cy.wait('@updateCall');
	});
	...
	it.skip('should render live score updates to a cricketblog', function () {
		...
		cy.visit(
			'/Article/web/https://theguardian.com/sport/live/2022/mar/27/west-indies-v-england-third-test-day-four-live?live',
		);
