...
export const sIndicator = async (): Promise<void> => {
	...
	const maybeProxiedStartingPoint = pathname.indexOf(
		'https://www.theguardian.com',
	); // for local dev

	const capiIdFromPath =
		maybeProxiedStartingPoint === -1
			? pathname
			: pathname.substring(maybeProxiedStartingPoint + 28);
	...
	async function findAndProcessOnwardLinks() {
		...
		const allOnwardAnchorElements = Array.from(
			document.querySelectorAll<HTMLElement>(
				"a[href^='/'],a[href^='https://www.theguardian.com/']",
			),
		);
		...
		>((acc, anchorElem) => {
			...
			const capiId = href.startsWith('https://www.theguardian.com/')
				? href.substring(28)
				: href.substring(1);
