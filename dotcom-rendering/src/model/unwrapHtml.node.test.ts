...
void nodeDescribe('unwrapHtml', () => {
	void nodeIt('Returns unwrapped HTML if prefix and suffix match', () => {
	...
	void nodeIt(
		'Returns non-unwrapped HTML if prefix and suffix do not match',
		() => {
			const bqUnwrap: Params = {
			...
			const { willUnwrap: isUnwrapped, unwrappedHtml } =
				unwrapHtml(bqUnwrap);
	...
	void nodeIt(
		'Returns wrapped HTML if prefix and suffix of one "fix" match from multiple options',
		() => {
			const bqUnwrap: Params = {
				html: '<blockquote><p>inner</p></blockquote>',
				fixes: [
