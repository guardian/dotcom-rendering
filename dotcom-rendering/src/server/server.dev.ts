...
renderer.all(/Article(\/\*)*/, handleArticle);
renderer.all(/ArticleJson(\/\*)*/, handleArticleJson);
renderer.all(/AMPArticle(\/\*)*/, handleAMPArticle);
renderer.all(/Interactive(\/\*)*/, handleInteractive);
renderer.all(/AMPInteractive(\/\*)*/, handleAMPArticle);
renderer.all(/Blocks(\/\*)*/, handleBlocks);
renderer.all(/Front(\/\*)*/, handleFront);
renderer.all(/FrontJSON(\/\*)*/, handleFrontJson);
renderer.all(/TagPage(\/\*)*/, handleTagPage);
renderer.all(/TagPageJSON(\/\*)*/, handleTagPageJson);
renderer.all(/EmailNewsletters(\/\*)*/, handleAllEditorialNewslettersPage);
renderer.all(/AppsArticle(\/\*)*/, handleAppsArticle);
renderer.all(/AppsInteractive(\/\*)*/, handleAppsInteractive);
renderer.all(/AppsBlocks(\/\*)*/, handleAppsBlocks);
renderer.all(/EditionsCrossword(\/\*)*/, handleEditionsCrossword);
renderer.all(/FootballMatchListPage(\/\*)*/, handleFootballMatchListPage);
renderer.all(/FootballTablesPage(\/\*)*/, handleFootballTablesPage);
renderer.all(/CricketMatchPage(\/\*)*/, handleCricketMatchPage);
renderer.all(/FootballMatchSummaryPage(\/\*)*/, handleFootballMatchPage);
