...
import type { AdTargeting, VideoEventKey } from '../../types/youtubeAtom';
...
const createInstantiateImaManager =
	...
	(player: YT.Player) => {
		...
		const customParams =
			// eslint-disable-next-line @typescript-eslint/strict-boolean-expressions, @typescript-eslint/no-unnecessary-condition -- TODO
...
export const YoutubeAtomPlayer = ({
	...
	useLayoutEffect(() => {
		 ...
		 */
		return () => {
			playerListenerNames.forEach((playerListener) => {
			...
			Object.entries(customListenersNames).forEach(
