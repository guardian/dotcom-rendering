...
const { sites, root, dist, target } = require('../../config');
...
const copyStatic = () => {
    ...
    return Promise.all(
        sites.map(site =>
            cpy(
                ['**/*'],
                path.resolve(target, 'frontend-static', 'guui', 'static', site),
                {
                    cwd: path.resolve(root, site, 'static'),
                    parents: true,
                    nodir: true,
                },
            ),
