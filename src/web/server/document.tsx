...
export const document = ({ data }: Props) => {
    ...
    const statsFile =
        process.env.NODE_ENV === 'production'
            ? path.resolve(`./loadable-stats.server.json`)
            : path.resolve(`dist/loadable-stats.server.json`);

    // We create an extractor from the statsFile
    const extractor = new ChunkExtractor({
        statsFile,
        entrypoints: ['frontend.server'],
    });
    ...
    const extractedScriptTags = extractor.getScriptTags(); // or extractor.getScriptElements();
